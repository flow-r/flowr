...
<p><strong>Version:</strong> 0.9.9.2</p>
...
to_flow(x, def, grp_col, jobname_col, cmd_col, flowname, flow_run_path, platform, containerize&nbsp;=&nbsp;TRUE, submit&nbsp;=&nbsp;FALSE, execute&nbsp;=&nbsp;FALSE, qobj, verbose&nbsp;=&nbsp;opts_flow$get("verbose"), ...)</div>
<div>## method for class 'data.frame'
to_flow(x, ...)</div>
<div>## method for class 'list'
to_flow(x, def, flowname, flow_run_path, desc, qobj, module_cmds&nbsp;=&nbsp;"", verbose&nbsp;=&nbsp;opts_flow$get("verbose"), ...)</div></code></pre>
...
<dl class="rd-dl">
  ...
  <dt>grp_col</dt>
  <dd class="rd-dd">column name used to split x (flow_mat) into multiple flows [samplename]</dd>
  <dt>jobname_col</dt>
  ...
  <dt>flow_run_path</dt>
  <dd class="rd-dd">base path to be used for this flow. 
...
it might be better and cleaner to create a containerize all of them
in a new date-stamped folder. [TRUE]</dd>
  <dt>submit</dt>
  <dd class="rd-dd">after creating a flow object, should flowr 
also use <a href=#submit_flow>submit_flow</a> to perform a dry-run OR real submission. [FALSE]
See below for details.</dd>
  <dt>execute</dt>
  <dd class="rd-dd">when calling <a href=#submit_flow>submit_flow</a>, 
should it execute it [TRUE] or perform a dry-run [FALSE]. See below for details.</dd>
  <dt>qobj</dt>
...
[<code>opts_flow$get("verbose")</code>] [1]</dd>
  <dt>desc</dt>
  <dd class="rd-dd">Advanced Use. final flow name.</dd>
  <dt>module_cmds</dt>
  <dd class="rd-dd">A character vector of additional commands, which will be prepended to each script of the flow.</dd>
</dl>
...
<li><code>queue</code> the queue to use for job submission [short]
  ...
  <p class="rd-p"><strong>NOTE:</strong> Guessing is a experimental feature, please check the definition carefully, 
and its provided to help and not replace best your best judgement. <br></p>
