...
<h4>Usage</h4>
<pre class="r"><code>to_flowmat(x, ...)

to_flowmatlist(x, samplename, ...)

to_flowmatdata.frame(x, ...)

to_flowmatflow(x, ...)

as.flowmat(x, grp_col, jobname_col, cmd_col, ...)

is.flowmat(x)</code></pre>
...
<h4>Usage</h4>
<pre class="r"><code>run(x, platform, def, flow_run_path = get_opts("flow_run_path"),
  execute = FALSE, ...)
...
<pre class="r"><code>flow(jobs = list(new("job")), name = "newflow", desc = "my_super_flow",
  ...
  flow_path = "", version = "0.0", status = "created", execute = "")</code></pre>
