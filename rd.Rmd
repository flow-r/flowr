...
The default value is picked up from the internal \textbfflowr.conf file.
To redefine this value, one could create a new file called \textbf~/flowr/conf/flowr.conf and
add a line:</p>

  <p class="rd-p"><code>flow_run_path TAB my_awesome_path</code>, where <code>TAB</code> is a tab character, since these are tab
seperated files.</p>

  <p class="rd-p">Also, at any time you can run, load_conf; to load custom options.</p>
